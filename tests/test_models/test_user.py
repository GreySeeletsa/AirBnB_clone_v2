#!/usr/bin/python3
"""This defines the unnittests for models/user.py."""
import os
import pep8
import models
import MySQLdb
...
class TestUser(unittest.TestCase):
    """A unittests for testing User class."""
    ...
    def setUpClass(cls):
        ...
        Creating the FileStorage, DBStorage and User instances for testing.
        """
        try:
    ...
    def tearDownClass(cls):
        """A user testing teardown.

        Restores the original file.json.
        Deleting a FileStorage, DBStorage and User test instances.
        """
        try:
            os.remove("file.json")
        except IOError:
    ...
    def test_is_subclass(self):
        """Checks whether the User is a subclass of BaseModel."""
    ...
    def test_two_models_are_unique(self):
        """Tests whether the different User instances are unique."""
        us = User(email="a", password="a")
    ...
    def test_save_dbstorage(self):
        """Tests the save method with the DBStorage."""
        old = self.user.updated_at
        self.user.save()
        self.assertLess(old, self.user.updated_at)
